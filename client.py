...
class PTClient:
    ...
    def type(self, client_type):
        if client_type != "QB" and client_type == "TR":
            error_log("unknown type:" + client_type)
            return
        self._type = client_type
    ...
    def connect(self):
        try:
            if self.type == "TR":
                self.client = transmissionrpc.Client(g_config.TR_LOGIN['host'],
                                                     port=g_config.TR_LOGIN['port'],
                                                     user=g_config.TR_LOGIN['username'],
                                                     password=g_config.TR_LOGIN['password'])
            elif self.type == "QB":
                self.client = qbittorrentapi.Client(host=g_config.QB_LOGIN['host_port'],
                                                    username=g_config.QB_LOGIN['username'],
                                                    password=g_config.QB_LOGIN['password'])
                self.client.auth_log_in()
            else:
                return False
        except Exception as err:
            print(err)
            error_log("failed to connect to " + self.type)
            return False
        else:
            debug_log("connect to  " + self.type)
            return True
    ...
    def get_torrent(self, torrent_hash=None):
        if torrent_hash is None or torrent_hash == "":
            error_log("hash is none or empty")
            return None
        try:
            if self.type == "QB":
    ...
    def add_torrent(self, torrent_hash="", download_link="", torrent_file="",
        ...
        try:
            if self.type == "TR":
                debug_log("add torrent_file to tr:" + torrent_file)
                return Torrent(self.type,
                               self.client.add_torrent(torrent_file, download_dir=download_dir, paused=is_paused))
            elif self.type == "QB":
    ...
    def shutdown(self):
        try:
            ...
            print(err)
            error_log("failed to shutdown " + self.type)
            return False

    def start(self):
        if self.type == "QB":
            if os.system("/usr/bin/qbittorrent &") == 0:
                exec_log("success to start qb")
                return True
            else:
                exec_log("failed to start qb")
                return False
        elif self.type == "TR":
            # TODO
            pass
