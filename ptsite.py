...
class NexusPage:
    ...
    def request_detail_page(self, torrent_id):
        if not self.site:
            return False
        self.detail_url = self.site['first_url'] + 'details.php?id=' + torrent_id + '&hit=1'
        ...
        try:
            ...
            print(err)
            exec_log("failed to request from " + self.detail_url)
            # self.Print("error:"+self.error_string)
        ...
        t_info = Info()
        # FRDS 详情页格式有些不一样
        if self.site['name'] == 'FRDS' and t_info.get_from_detail(self.soup):
            self.info = t_info
            debug_log("success to get from detail:" + self.detail_url)
            return True

        summary_anchor = self.soup.find('div', id='kdescr')
        if summary_anchor:
            t_summary = summary_anchor.get_text()
        else:
            site_log("not find summary:" + self.detail_url)
            return False
        site_log(t_summary)

        t_info.get_from_summary(t_summary)
        if t_info.douban_id != "" or t_info.imdb_id != "":
            self.info = t_info
            debug_log("success to get from detail:" + self.detail_url)
            return True
        else:
            exec_log("failed to get from detail:" + self.detail_url)
            return False
    ...
    def request_free_page(self):
        ...
        if res.text.find("次連續登錄失敗將導致你的IP位址被禁用") >= 0:
            error_log(f"{self.site['name']} cookie失效")
        """
        ...
        if len(self.processed_list) == 0:
            exec_log("can not find processed_list " + self.site['name'])
            self.set_error_count(False)
            return False
        else:
            self.set_error_count(True)
    ...
    def to_be_downloaded(self, title, free):
        ...
        if self.site.get('auto') is None and self.site.get('manual') is None:
            if free:
        ...
        if auto_keywords is not None:  # 如果不配置，就跳过
            if filter_by_keywords(title, auto_keywords):
                if free:
                    return AUTO_DOWNLOAD  # 种子为free，则自动加入
                elif not auto_keywords.get('free'):
                    return AUTO_DOWNLOAD  # 种子不为free,但'free'关键字配置为false，则也自动加入
                else:
                    pass
        ...
        if manual_keywords is not None:
            if filter_by_keywords(title, manual_keywords):
    ...
    def get_id_from_detail(rss_name, detail_url):
        ...
        if rss_name == "" or detail_url == "":
            error_log("rss_name or detail_url is null")
            return_dict['error_string'] = "rss_name or detail_url is null"
            return return_dict

        # 获取站点
        t_site = NexusPage.get_site(rss_name)
        if t_site is None:
            error_log("unknown site name:" + rss_name)
            return_dict['error_string'] = f"unknown site name:{rss_name}"
            return return_dict
        debug_log("find site:{}".format(t_site['name']))

        site_log(detail_url)

        cookie_dict = {"cookie": t_site['cookie']}
        s = requests.Session()
        s.cookies.update(cookie_dict)
        myheaders = {
            'cookie': t_site['cookie'],
            'user-agent': NexusPage.user_agent}
        if t_site.get('referer'):
            myheaders['referer'] = t_site.get('referer')
        if t_site.get('host'):
        ...
        try:
            ...
            print(err)
            exec_log("failed to request from " + detail_url)
            return_dict['return_code'] = RETRY
            return_dict['error_string'] = str(err)
            # self.Print("error:"+self.error_string)
            # self.set_error_count(False)
            return return_dict
        ...
        if t_site['name'] == 'FRDS':
            ...
            if return_code:
                debug_log("success to get from detail:" + detail_url)
                return_dict['return_code'] = OK
                return_dict['douban_id'] = douban_id
        ...
        if not summary_anchor:
            site_log("not find summary:" + detail_url)
            return_dict['error_string'] = f'not find summary:{detail_url}'
            return return_dict
        t_summary = summary_anchor.get_text()
        site_log("summary===")
        site_log(t_summary)
