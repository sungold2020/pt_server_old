...
g_CheckDisk = ""
gCheckTime = ""
DiskList=["wd2t","wd2t-2","sg3t","sg3t-2","wd4t","sg8t"]
gDictList = (\
        ("Crime","犯罪"),\
        ("Drama","剧情"),\
        ("History","历史"),\
...
class TMMInfo:
    ...
    def UpdateTable(self)   :
        ...
        #已经存在就update
            ...
            #imdbID和Genre 为空的话，采用tmm进行更新表
            if imdbID      == ""      : tUpdated += 1; imdbID = self.imdbID
            if Genre       == ""      : tUpdated += 1; Genre  = self.Genre                
            if tUpdated >= 1:
...
def CreateLink(tMovie,Disk):
    ...
    if os.path.exists(destdir):
        ...
        #检查其他disk下是否存在该目录，如果存在就直接拷贝过来
        IsFind = False
        for tDisk in DiskList:
            destdir2=os.path.join(os.path.join(LinkDir,tDisk),DirName)
            if os.path.exists(destdir2):
                try:
                    shutil.move(destdir2, os.path.join(LinkDir,Disk))
                except Exception as err:
                    print(err)
                    print("failed to mv dir:"+destdir2)
                else:
                    print("success mv dir:"+destdir2)
                    IsFind = True
        if IsFind == False:
        ...
        if tempfile[-4:] == '.mkv' or\
           ...
           tempfile[-3:] == '.ts' :
            ...
            if os.path.islink(destfullfile):
                if os.readlink(destfullfile) == srcfullfile : continue
                else:
                    try:
                        os.remove(destfullfile)
                    except Exception as err:
                        print(err)
                        print("failed to remove link:"+destfullfile)
                    else:
                        print("success remove link:"+destfullfile)
            #create link
            try:
